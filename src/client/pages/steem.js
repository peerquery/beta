...
async function getState(path) {
    try {
        ...
        document.getElementById('block').innerText =
            '#' + result.props.head_block_number;
        document.getElementById('version').innerText =
            result.witness_schedule.majority_version;

        document.getElementById('witness-href').href =
            '/@' + result.props.current_witness;
        document.getElementById('witness_img').src =
            'https://img.busy.org/@' + result.props.current_witness;
        document.getElementById(
            'witness'
        ).innerText = result.props.current_witness.toUpperCase();
        ...
        var process = async (result, path) => {
            ...
            if (path == '/trending/') sort = result.discussion_idx[n].trending;
            if (path == '/created/') sort = result.discussion_idx[n].created;
            if (path == '/hot/') sort = result.discussion_idx[n].hot;
            if (path == '/promoted/') sort = result.discussion_idx[n].promoted;
...
(async () => {
    try {
        var result = await client.database.call('get_trending_tags', [
            undefined,
            41,
        ]);
        for (var x in result) {
