...
(async () => {
    try {
        ...
        if (new URLSearchParams(document.location.search).get('access_token')) {
            ...
            let auth_obj = {};
            (auth_obj.access_token = new URLSearchParams(
                document.location.search
            ).get('access_token')),
            (auth_obj.username = new URLSearchParams(
                document.location.search
            ).get('username')),
            (auth_obj.expires_in = new URLSearchParams(
                document.location.search
            ).get('expires_in'));
            ...
            var redirect_uri = new URLSearchParams(
                document.location.search
            ).get('state');
            ...
            !new URLSearchParams(document.location.search).get('access_token')
        ) {
