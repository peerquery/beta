...
module.exports = {
    ...
    check_vote: async function(resp, voteArray) {
        for (var x in voteArray) {
            if (voteArray[x].voter == active_user && voteArray[x].percent > 0) {
                document.getElementById(resp.id + '-percent').innerHTML =
                    '<span class=\'desktop-only\'> Voted at: </span>' +
                    voteArray[x].percent / 100 +
                    '%';
                document.getElementById(resp.id + '-btn').dataset.votestate =
                    'true';
                document.getElementById(resp.id + '-btn').dataset.value = '0';
                document.getElementById(resp.id + '-range').value =
                    voteArray[x].percent;
        ...
        document.getElementById(resp.id + '-voterlist').dataset.html =
            '<div>' + (await this.list_voters(voteArray)) + '</div>';
    },

    check_response_votes: async function(response, netVotes, activeVotes) {
        if (netVotes > 0) {
            document.getElementById(response.id + '-rescount').innerText =
                ' +' + this.count_votes(activeVotes);
        } else if (netVotes < 0) {
            document.getElementById(response.id + '-rescount').innerText =
                ' -' + this.count_votes(activeVotes);
        } else if (netVotes == 0) {
    ...
    check_cash: function(pst) {
        return Math.max(
            Number(pst.pending_payout_value.split(' ')[0]),
            Number(pst.total_payout_value.split(' ')[0]) +
                Number(pst.curator_payout_value.split(' ')[0])
        ).toLocaleString();
    },
    ...
    link_issues: function(text) {
        text.replace(/(^|\s)(#[-a-z\d]+)/gi, tag => {
            ...
            return (
                space +
                '<a target="_blank" href="https://steemit.com/trending/' +
                tagLower +
                ' ">' +
                tag +
                ' </a>'
    ...
    link_mentions: function(text) {
        text.replace(
            ...
            (match, preceeding1, preceeding2, user) => {
                ...
                //const valid = validate_account_name(userLower) == null; //build validate_account_name() function later
