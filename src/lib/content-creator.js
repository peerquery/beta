...
module.exports = {
    post: async function(text) {
        ...
        author_img.src =
            'https://steemitimages.com/u/' + text.author + '/avatar';
        ...
        author_rep_div.innerText = await utils.reputation(
            text.author_reputation
        );
        ...
        i_resteem.className =
            'small circular pink inverted fitted refresh icon';
        ...
        span_earned.innerText = Math.max(
            Number(text.pending_payout_value.split(' ')[0]),
            Number(text.total_payout_value.split(' ')[0]) +
                Number(text.curator_payout_value.split(' ')[0])
        ).toLocaleString();
        ...
        span_votes.innerText = Number(
            text.active_votes.length
        ).toLocaleString();
    ...
    comment_summary: async function(cmt) {
        ...
        img_avatar.onerror = function() {
            this.src = '/assets/images/placeholder.png';
            this.onerror = '';
        };
        img_avatar.src =
            'https://steemitimages.com/u/' + cmt.author + '/avatar';
        ...
        a_author.innerText =
            ...
            cmt.author +
            '(' +
            (await utils.reputation(cmt.author_reputation)) +
            ')';
        ...
        var span_earned = document.createElement('span');
        span_earned.innerText = Math.max(
            Number(cmt.pending_payout_value.split(' ')[0]),
            Number(cmt.total_payout_value.split(' ')[0]) +
                Number(cmt.curator_payout_value.split(' ')[0])
        ).toLocaleString();
    ...
    tag: async function(tag, num) {
        ...
        label2.innerText =
            '$' + Number(tag.total_payouts.split('.')[0]).toLocaleString();
    ...
    comment: async function(response) {
        ...
        au_author.innerText =
            '@' +
            response.author.toUpperCase() +
            '(' +
            utils.reputation(response.author_reputation) +
            ')';
        ...
        vote_div.className =
            'ui clearing segment middle aligned stackable grid';
        ...
        div_vote_toggle.className =
            'ui left labeled vote_toggle right floated tiny button';
        div_vote_toggle.setAttribute('tabindex', '0');
        div_vote_toggle.id = response.id;

        var comment_earned = Math.max(
            Number(response.pending_payout_value.split(' ')[0]),
            Number(response.total_payout_value.split(' ')[0]) +
                Number(response.curator_payout_value.split(' ')[0])
        ).toLocaleString();
    ...
    query: async function(data) {
        ...
        if (data.project_slug_id) {
            spanLast.innerHTML =
                'Affiliation: <a href=\'/project/' +
                data.project_slug_id +
                '\'>' +
                data.project_title +
                '</a>';
        } else {
