...
module.exports = function (app) {
	...
	(async () => {
		try{
			...
			var reports = await reportSchema.find({}, 'permlink').sort({'_id': 1}).limit(100);
			var queries = await querySchema.find({}, 'permlink').sort({'_id': 1}).limit(100);
			var projects = await projectSchema.find({}, 'slug').sort({'_id': 1}).limit(100);
			var peers = await peerSchema.find({}, 'account').sort({'_id': 1}).limit(100);
    ...
    app.get( "/sitemap_index.xml", async (req, res) => {
		...
		var report = await reportSchema.findOne({}, '_id created').sort({"_id": 1});
		var query = await querySchema.findOne({}, '_id created').sort({"_id": 1});
		var project = await projectSchema.findOne({}, '_id created').sort({"_id": 1});
		...
		sm_i = sm_i.replace(/LAST_UPDATED_PROJECTS/g, project.created || new Date() );
		sm_i = sm_i.replace(/LAST_UPDATED_REPORTS/g, report.created || new Date() );
		sm_i = sm_i.replace(/LAST_UPDATED_QUERIES/g, query.created || new Date() );
    ...
    app.get( "/report-sitemap.xml", async (req, res) => {
		...
		try{
			if (reportSitemap.isCacheValid()){
				...
				var xml = await reportSitemap.toXML();
				...
				var reports = await reportSchema.find({}, 'permlink category').sort({'_id': 1}).limit(100);
    ...
    app.get( "/peer-sitemap.xml", async (req, res) => {
		...
		try{
			if (peerSitemap.isCacheValid()){
				...
				var xml = await peerSitemap.toXML();
				res.header('Content-Type', 'application/xml');
				res.send(xml);
    ...
    app.get( "/project-sitemap.xml", async (req, res) => {
		...
		try{
			if (projectSitemap.isCacheValid()){
				...
				var xml = await projectSitemap.toXML();
				res.header('Content-Type', 'application/xml');
				res.send(xml);
    ...
    app.get( "/query-sitemap.xml", async (req, res) => {
		...
		try{
			if (querySitemap.isCacheValid()){
				...
				var xml = await querySitemap.toXML();
				res.header('Content-Type', 'application/xml');
				res.send(xml);
