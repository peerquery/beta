        ...
        try {
            ...
            if (client_account == sc2_account) {
                ...
                if (typeof sc2_user.account.json_metadata === 'string') {
                    ...
                    var json_metadata = await asyncParser(
                        sc2_user.account.json_metadata
                    );

                    if (
                        typeof json_metadata.profile !== 'undefined' &&
                        typeof json_metadata.profile.about === 'string'
                    )
                        about = json_metadata.profile.about;
                    if (
                        typeof json_metadata.profile !== 'undefined' &&
                        typeof json_metadata.profile.location === 'string'
                    )
                        user_location = json_metadata.profile.location;
                }
                ...
                var options = {
                    ...
                    new: true,
                    runValidators: true,
                    fields: { login_count: 1, id: -1 },
                };
                var response = await peer.findOneAndUpdate(
                    query,
                    newPeer,
                    options
                );
                ...
                if (response.login_count == 1) {
                    ...
                    action = 'sign_up';
                    description =
                        '@' + client_account + ' just joined the community';
                ...
                var secured_access_token = await encryptor.encrypt(
                    client_token,
                ...
                res.status(403)
                    .clearCookie('_auth')
                    .send('sorry, invalid auth details');
            }
        } catch (err) {
            //err processing auth
            res.status(500)
                .clearCookie('_auth')
                .send('sorry, error processing login');
            console.log(err);
        }
    });
