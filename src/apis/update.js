...
module.exports = function (app) {
    ...
    app.post('/api/private/update/project', async function (req, res) {
        ...
        try {
            ...
            var story = await DOMPurify.sanitize(req.body.story, {SAFE_FOR_JQUERY: true});
            ...
            var newActivity = activity({
                ...
                action: 'update',
                type: 'project',
                description: '@' + req.active_user.account + ' just updated their project: ' + req.body.name,
                created: Date.now()
            ...
            var _status = await project.findOneAndUpdate(query, newProject, options);
			
            if (!_status) return res.status(500).send('sorry, could not create project. please try again');
